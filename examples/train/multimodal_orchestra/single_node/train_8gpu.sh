...
if [ "$enable_agent" = "True" ]; then
    run_name="exp13_${reward_manager}-${strategy}-agent-${model_pretty_name}-${run_name_postfix}-${rl_alg}-lr${lr}-c_and_f_s-${correct_and_format_only_actor_steps}-cu_s-${clean_up_skill_step}-l_s-${LATENCY_PENALTY_START_STEP}-t_s-${TOOL_PENALTY_START_STEP}-kl_l-${kl_loss_coef}"
else
