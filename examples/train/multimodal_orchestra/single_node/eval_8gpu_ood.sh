#!/usr/bin/env bash
# =============================================================================
# Multimodal Orchestra Eval Script - Single Node (8 GPUs)
# =============================================================================
# 单节点8卡评测脚本，使用 uvicorn 启动工具服务器
# 
# 用法:
#   cd /path/to/verl_duo
...
export SWANLAB_LOG_DIR=${ROOT_DIR}/swanlog_output_clevr_math_tool_ood
...
if [ -z "$TRAIN_DATA_PATH" ]; then
    echo "TRAIN_DATA_PATH must be set to a non-empty parquet path because the trainer always builds train_dataset." >&2
    exit 1
fi

if [ -z "$VAL_DATA_PATH" ]; then
    echo "VAL_DATA_PATH must be set to the evaluation parquet path." >&2
    exit 1
fi
...
# Reward Manager配置 - 使用 eval 专用宽松答案提取版本
reward_manager=multimodal_orchestra_eval_relaxed_answer
...
run_name_postfix="multimodal-orchestra-adapt-skills-${EVAL_TOOL_VARIANT}-${model_tag}"

if [ "$enable_agent" = "True" ]; then
    run_name="eval-${reward_manager}-${strategy}-agent-${model_pretty_name}-${rl_alg}-n${n}-b${batch_size}-t${temperature}-lr${lr}-${run_name_postfix}"
else
    run_name="eval-${reward_manager}-${strategy}-${model_pretty_name}-${rl_alg}-n${n}-b${batch_size}-t${temperature}-lr${lr}-${run_name_postfix}"
fi
...
# =============================================================================
echo "=========================================="
echo "Starting Evaluation (val-only)"
echo "Run Name: $run_name"
echo "Run ID: $RUN_ID"
echo "Eval Tool Variant: $EVAL_TOOL_VARIANT"
echo "Train Data Path: $TRAIN_DATA_PATH"
echo "Eval Data Path: $VAL_DATA_PATH"
