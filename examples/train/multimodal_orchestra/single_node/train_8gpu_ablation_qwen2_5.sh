#!/usr/bin/env bash
# =============================================================================
# Multimodal Orchestra Training Script - Single Node (8 GPUs)
# =============================================================================
# 单节点8卡训练脚本，使用 uvicorn 启动工具服务器
# 
# 用法:
#   cd /path/to/verl_duo
...
export SWANLAB_LOG_DIR=${ROOT_DIR}/swanlog_output_ablation_qwen2.5
...
# 模型和训练参数配置
# =============================================================================
model_name='/inspire/hdd/project/ai4education/zhouaimin-p-zhouaimin/zc/verltools/model/Qwen2.5-VL-7B-Instruct'
rl_alg=grpo
n_gpus_per_node=8
n_nodes=1
n=8
...
reward_manager=multimodal_orchestra_ablate_latency_format_skill
...
if [ "$enable_agent" = "True" ]; then
    run_name="ablation_qwen25_7b_${reward_manager}-${run_name_postfix}-c_and_f_s-${correct_and_format_only_actor_steps}-cu_s-${clean_up_skill_step}-l_s-${LATENCY_PENALTY_START_STEP}-t_s-${TOOL_PENALTY_START_STEP}-kl_l-${kl_loss_coef}"
else
    run_name="exp13_${reward_manager}-${strategy}-${model_pretty_name}-${run_name_postfix}-${rl_alg}-lr${lr}-c_and_f_s-${correct_and_format_only_actor_steps}-cu_s-${clean_up_skill_step}-l_s-${LATENCY_PENALTY_START_STEP}-t_s-${TOOL_PENALTY_START_STEP}-kl_l-${kl_loss_coef}"
fi
...
PYTHONUNBUFFERED=1 python3 -m verl_tool.trainer.main_ppo \
    ...
    +reward_model.reward_kwargs.enable_latency_reward=True \
