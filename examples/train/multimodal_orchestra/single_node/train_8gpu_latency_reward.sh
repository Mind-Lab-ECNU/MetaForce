...
# 序列长度配置
max_prompt_length=$((8192*2 + 4096))
max_response_length=$((8192*2 + 1024))
...
LATENCY_REWARD_START_STEP=${LATENCY_REWARD_START_STEP:-0}
...
if [ "$enable_agent" = "True" ]; then
    run_name="exp13-agent-${model_pretty_name}-${run_name_postfix}-${rl_alg}-lr${lr}-c_and_f_s-${correct_and_format_only_actor_steps}-cu_s-${clean_up_skill_step}-l_r_s-${LATENCY_REWARD_START_STEP}-t_s-${TOOL_PENALTY_START_STEP}-kl_l-${kl_loss_coef}"
else
    run_name="exp13_${reward_manager}-${strategy}-${model_pretty_name}-${run_name_postfix}-${rl_alg}-lr${lr}-c_and_f_s-${correct_and_format_only_actor_steps}-cu_s-${clean_up_skill_step}-l_s-${LATENCY_REWARD_START_STEP}-t_s-${TOOL_PENALTY_START_STEP}-kl_l-${kl_loss_coef}"
fi
...
PYTHONUNBUFFERED=1 python3 -m verl_tool.trainer.main_ppo \
    ...
    +reward_model.reward_kwargs.latency_reward_start_step=$LATENCY_REWARD_START_STEP \
