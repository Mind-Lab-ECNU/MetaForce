...
if [ "$enable_agent" = "True" ]; then
    run_name="ablation_no_format_reward_${reward_manager}-${strategy}-agent-${model_pretty_name}-${rl_alg}-lr${lr}-${run_name_postfix}-c_and_f_s-${correct_and_format_only_actor_steps}-cu_s-${clean_up_skill_step}-l_s-${LATENCY_PENALTY_START_STEP}-t_s-${TOOL_PENALTY_START_STEP}-kl_l-${kl_loss_coef}"
else
