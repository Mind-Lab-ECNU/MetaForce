...
# 训练数据配置 (运行前直接在这里填写)
# =============================================================================
TRAIN_DATA_PATH='/inspire/hdd/project/ai4education/public/wsa_1.0/verltools/verl_m/data_multi_category/data/train_merge_v6_tool_id_think/train_merge_v6_tool_id_think.parquet'
VAL_DATA_PATH='/inspire/hdd/project/ai4education/public/wsa_1.0/verltools/verl_m/data_multi_category/data/train_merge_v6_tool_id_think/train_merge_v6_tool_id_think.parquet'
...
# 模型和训练参数配置
# =============================================================================
model_name="/inspire/hdd/project/ai4education/zhouaimin-p-zhouaimin/zc/verltools/model/Qwen3-VL-8B-Thinking"
rl_alg=grpo
n_gpus_per_node=4
n_nodes=1
n=8
...
if [ "$enable_agent" = "True" ]; then
    run_name="exp16_think_${reward_manager}-${strategy}-agent-${model_pretty_name}-${rl_alg}-lr${lr}-${run_name_postfix}-c_and_f_s-${correct_and_format_only_actor_steps}-cu_s-${clean_up_skill_step}-l_s-${LATENCY_PENALTY_START_STEP}-t_s-${TOOL_PENALTY_START_STEP}-kl_l-${kl_loss_coef}"
else
    run_name="exp16_${reward_manager}-${strategy}-${model_pretty_name}-${rl_alg}-lr${lr}-${run_name_postfix}-c_and_f_s-${correct_and_format_only_actor_steps}-cu_s-${clean_up_skill_step}-l_s-${LATENCY_PENALTY_START_STEP}-t_s-${TOOL_PENALTY_START_STEP}-kl_l-${kl_loss_coef}"
fi
