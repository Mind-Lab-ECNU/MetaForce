#!/usr/bin/env bash
# =============================================================================
# Multimodal Orchestra Single-Node Training Wrapper with Retry (4 GPUs)
# =============================================================================
# 启动方式:
#
# 1) 一体化模式（默认，脚本内部启动 tool server）:
#   MODE=bundled bash examples/train/multimodal_orchestra/single_node/train_4gpu_with_retry.sh
#
# 2) 外置工具模式:
#   MODE=external TOOL_SERVER_URL=http://<tool_host>:<tool_port>/get_observation \
#     bash examples/train/multimodal_orchestra/single_node/train_4gpu_with_retry.sh
#
# 3) 自动模式（默认）:
...
RETRY_LOG_DIR=${RETRY_LOG_DIR:-"$(pwd)/logs/multimodal_orchestra_single_node/retry-ablation"}

mkdir -p "$RETRY_LOG_DIR"

BUNDLED_SCRIPT="${SCRIPT_DIR}/train_4gpu_ablation.sh"
EXTERNAL_SCRIPT="${SCRIPT_DIR}/train_4gpu_no_tool.sh"
...
while true; do
    ...
    attempt_log="${RETRY_LOG_DIR}/single_train_4gpu_attempt${attempt}_${attempt_ts}.log"
