#!/usr/bin/env bash
# =============================================================================
# Multimodal Orchestra Training Script - Single Node (8 GPUs, InternVL)
# =============================================================================
# 单节点8卡 InternVL 训练脚本。
# 在原 train_8gpu.sh 基础上增加：
# - InternVL model_family 显式指定
# - vLLM skip_mm_profiling，避免 InternVL 启动期多模态 profiling 崩溃
# - 默认模型切到 InternVL3.5-14B
# 
# 用法:
#   cd /path/to/verl_duo
#   bash examples/train/multimodal_orchestra/single_node/train_8gpu_internvl14b.sh
# =============================================================================
...
export SWANLAB_LOG_DIR=${ROOT_DIR}/swanlog_output_internvl
...
# 模型和训练参数配置
# =============================================================================
model_name="${MODEL_NAME:-/inspire/hdd/project/ai4education/zhouaimin-p-zhouaimin/zc/verltools/model/InternVL3_5-14B-HF}"
model_family="${MODEL_FAMILY:-internvl}"
rl_alg=grpo
n_gpus_per_node=8
n_nodes=1
n=8
...
max_response_length=$((8192*2))
max_action_length=8192
max_obs_length=$((8192*2))
...
run_name_postfix="multimodal-orchestra-adapt-skills"

if [ "$enable_agent" = "True" ]; then
    run_name="exp17_${reward_manager}-${strategy}-agent-${model_pretty_name}-${rl_alg}-lr${lr}-${run_name_postfix}-c_and_f_s-${correct_and_format_only_actor_steps}-cu_s-${clean_up_skill_step}-l_s-${LATENCY_PENALTY_START_STEP}-t_s-${TOOL_PENALTY_START_STEP}-kl_l-${kl_loss_coef}"
else
    run_name="exp17_${reward_manager}-${strategy}-${model_pretty_name}-${rl_alg}-lr${lr}-${run_name_postfix}-c_and_f_s-${correct_and_format_only_actor_steps}-cu_s-${clean_up_skill_step}-l_s-${LATENCY_PENALTY_START_STEP}-t_s-${TOOL_PENALTY_START_STEP}-kl_l-${kl_loss_coef}"
fi
...
PYTHONUNBUFFERED=1 python3 -m verl_tool.trainer.main_ppo \
    ...
    actor_rollout_ref.agent.tool_call_timeout=750 \
    ...
    +actor_rollout_ref.agent.model_family=$model_family \
