...
if [ "$enable_agent" = "True" ]; then
    run_name="exp15-agent-${model_pretty_name}-${rl_alg}-lr${lr}-${run_name_postfix}-c_and_f_s-${correct_and_format_only_actor_steps}-cu_s-${clean_up_skill_step}-l_r_s-${LATENCY_REWARD_START_STEP}-t_s-${TOOL_PENALTY_START_STEP}-kl_l-${kl_loss_coef}"
else
    run_name="exp15-${model_pretty_name}-${rl_alg}-lr${lr}-${run_name_postfix}-c_and_f_s-${correct_and_format_only_actor_steps}-cu_s-${clean_up_skill_step}-l_s-${LATENCY_REWARD_START_STEP}-t_s-${TOOL_PENALTY_START_STEP}-kl_l-${kl_loss_coef}"
fi
