#!/usr/bin/env bash
# =============================================================================
# Multimodal Orchestra Training Wrapper with Retry (4 Nodes x 8 GPUs = 32 GPUs)
# =============================================================================
# 启动方式（推荐四台机器都运行 wrapper）:
#
# 1) 一体化模式（head 本机起工具服务）:
#   Head:
#     NODE_RANK=0 MODE=bundled bash examples/train/multimodal_orchestra/multi_node/train_4node_32gpu_with_retry.sh
#   Worker:
#     NODE_RANK=1 MASTER_ADDR=10.247.75.123 MODE=bundled bash examples/train/multimodal_orchestra/multi_node/train_4node_32gpu_with_retry.sh
#     NODE_RANK=2 MASTER_ADDR=10.247.75.123 MODE=bundled bash examples/train/multimodal_orchestra/multi_node/train_4node_32gpu_with_retry.sh
#     NODE_RANK=3 MASTER_ADDR=10.247.75.123 MODE=bundled bash examples/train/multimodal_orchestra/multi_node/train_4node_32gpu_with_retry.sh
#     
# 
# 2) 外置工具模式:
#   Head:
#     NODE_RANK=0 MODE=external TOOL_SERVER_URL=http://<tool_host>:<tool_port>/get_observation \
#       bash examples/train/multimodal_orchestra/multi_node/train_4node_32gpu_with_retry.sh
#   Worker:
...
BUNDLED_SCRIPT="${SCRIPT_DIR}/train_4node_32gpu.sh"
EXTERNAL_SCRIPT="${SCRIPT_DIR}/train_4node_32gpu_no_tool.sh"
...
while true; do
    ...
    attempt_log="${RETRY_LOG_DIR}/train_rank${NODE_RANK}_attempt${attempt}_${attempt_ts}.log"
