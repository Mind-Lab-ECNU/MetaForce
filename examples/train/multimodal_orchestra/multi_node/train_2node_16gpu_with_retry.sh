#!/usr/bin/env bash
# =============================================================================
# Multimodal Orchestra Training Wrapper with Retry (2 Nodes x 8 GPUs = 16 GPUs)
# =============================================================================
# 启动方式（推荐两台机器都运行 wrapper）:
#
# 1) 一体化模式（head 本机起工具服务）:
#   Head:
#     NODE_RANK=0 MODE=bundled bash examples/train/multimodal_orchestra/multi_node/train_2node_16gpu_with_retry.sh
#   Worker:
#     00 NODE_RANK=1 MASTER_ADDR=252.0.198.67 MODE=bundled bash examples/train/multimodal_orchestra/multi_node/train_2node_16gpu_with_retry.sh
#     04 NODE_RANK=1 MASTER_ADDR=252.0.72.196 MODE=bundled bash examples/train/multimodal_orchestra/multi_node/train_2node_16gpu_with_retry.sh
#     step3 NODE_RANK=1 MASTER_ADDR=10.247.84.83 MODE=bundled bash examples/train/multimodal_orchestra/multi_node/train_2node_16gpu_with_retry.sh 
# 2) 外置工具模式:
#   Head:
...
BUNDLED_SCRIPT="${SCRIPT_DIR}/train_2node_16gpu.sh"
EXTERNAL_SCRIPT="${SCRIPT_DIR}/train_2node_16gpu_no_tool.sh"
