#!/usr/bin/env bash
# =============================================================================
# Multimodal Orchestra Training Script - Multi Node (2 Nodes x 8 GPUs = 16 GPUs)
# =============================================================================
...
#   NODE_RANK=0 bash '/inspire/hdd/project/ai4education/public/wsa_1.0/verltools/verl_m/examples/train/multimodal_orchestra/multi_node/train_2node_16gpu.sh'
#
#   # 在 worker 节点:
#   NODE_RANK=1 MASTER_ADDR=252.1.40.63 bash '/inspire/hdd/project/ai4education/public/wsa_1.0/verltools/verl_m/examples/train/multimodal_orchestra/multi_node/train_2node_16gpu.sh'
# =============================================================================
...
if [ "$NODE_RANK" -ne 0 ] && [ -z "$MASTER_ADDR" ]; then
    ...
    echo "Usage: NODE_RANK=1 MASTER_ADDR=<head_ip> bash train_2node_16gpu.sh"
    exit 1
fi
...
export CORRECT_AND_FORMAT_ONLY_ACTOR_STEPS=32
# =============================================================================
# SwanLab 本地配置
# =============================================================================
...
clean_up_skill_step=${CLEAN_UP_SKILL_STEP:-0}

# Reward Manager配置
reward_manager=multimodal_orchestra
LATENCY_PENALTY_START_STEP=${LATENCY_PENALTY_START_STEP:-64}
TOOL_PENALTY_START_STEP=${TOOL_PENALTY_START_STEP:-32}
...
run_name_postfix="multimodal-orchestra-2node-adapt-skills-id"

if [ "$enable_agent" = "True" ]; then
    run_name="exp10_${reward_manager}-${strategy}-agent-${model_pretty_name}-${rl_alg}-lr${lr}-${run_name_postfix}-c_and_f_s-${correct_and_format_only_actor_steps}-cu_s-${clean_up_skill_step}-l_s-${LATENCY_PENALTY_START_STEP}-t_s-${TOOL_PENALTY_START_STEP}-kl_l-${kl_loss_coef}"
else
    run_name="exp10_${reward_manager}-${strategy}-${model_pretty_name}-${rl_alg}-lr${lr}-${run_name_postfix}-c_and_f_s-${correct_and_format_only_actor_steps}-cu_s-${clean_up_skill_step}-l_s-${LATENCY_PENALTY_START_STEP}-t_s-${TOOL_PENALTY_START_STEP}-kl_l-${kl_loss_coef}"
fi
