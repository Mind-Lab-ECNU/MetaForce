...
class VerlToolAgentLoop(AgentLoopBase):
    ...
    def init_class(cls, config, tokenizer, processor, **kwargs):
        with cls._init_lock:
            ...
            cls.turn_end_token_ids = cls.tokenizer.encode(cls.agent_config.turn_end_token, add_special_tokens=False)
            ...
            configured_model_family = str(getattr(rollout_config.agent, "model_family", "auto"))
            trust_remote_code = bool(rollout_config.model.get("trust_remote_code", False))
            model_path = str(rollout_config.model.path)
            model_config = load_model_config(model_path, trust_remote_code)
            cls.model_family = resolve_model_family(
    ...
    async def run(self, sampling_params: dict[str, Any], **kwargs) -> AgentLoopOutput:
        ...
        stats_dict = {
            ...
            "valid_traj": 1,
            "retokenization_diff": [],
            "agent_prompt_lengths": [],
            "agent_output_lengths": [],
        }

        prompt_state = None
        if is_intern_family(self.model_family):
            prompt_state = InternVLPromptState(
                tokenizer=self.tokenizer,
                processor=self.processor,
                mtrl_role=self.agent_config.mtrl_role,
                sample_kwargs=kwargs,
            )
            generation_input = prompt_state.get_generation_input()
            prompt_ids = generation_input.prompt_ids
            image_data = generation_input.image_data
        elif image_data or audio_data:
        ...
        # Keep prompt width consistent with rollout padding to avoid batch cat failures.
        prompt_cap = self.config.actor_rollout_ref.rollout.prompt_length
        if prompt_cap is not None:
            prompt_cap = int(prompt_cap)
            orig_prompt_len = len(prompt_ids)
            if orig_prompt_len > prompt_cap:
                truncation = self.config.data.get("truncation", "left")
                if truncation == "left":
                    prompt_ids = prompt_ids[-prompt_cap:]
                elif truncation == "right":
                    prompt_ids = prompt_ids[:prompt_cap]
                elif truncation == "middle":
                    left_half = prompt_cap // 2
                    right_half = prompt_cap - left_half
                    prompt_ids = prompt_ids[:left_half] + prompt_ids[-right_half:]
                else:
                    raise ValueError(
                        f"Invalid data.truncation={truncation}. Expected one of left/right/middle."
                    )
                logger.warning(
                    "Truncated overlong prompt_ids in agent loop: traj_id=%s orig_len=%d cap=%d truncation=%s",
                    request_id,
                    orig_prompt_len,
                    prompt_cap,
                    truncation,
        ...
        for step in range(max_turns + 1):
            ...
            prompt_text_for_generation = None
            if prompt_state is not None:
                generation_input = prompt_state.get_generation_input()
                prompt_text_for_generation = generation_input.prompt_text
                # logger.warning(
                #     "intern prompt tail: traj_id=%s turn=%s tail=%r",
                #     request_id,
                #     step,
                #     prompt_text_for_generation[-400:],
                # )
            with simple_timer("generate_sequences", metrics):
                output = await self.server_manager.generate(
                    request_id=request_id,
                    prompt_ids=None if prompt_text_for_generation is not None else running_prompt_ids,
            ...
            stats_dict["format_messages"].append({"role": "assistant", "content": gen_text})
            if prompt_state is not None:
                prompt_state.append_assistant_text(gen_text)
            running_prompt_ids.extend(gen_ids)
            response_mask.extend([1] * len(gen_ids))
            response_logprobs.extend(gen_logprobs)
            # Track token lengths for cost attribution in downstream analysis.
            stats_dict["agent_prompt_lengths"].append(previous_length)
            stats_dict["agent_output_lengths"].append(len(gen_ids))
            ...
            if do_action and not is_last_step:
                ...
                if running_image_data is not None:
                    extra_fields["images"] = [encode_image_url(img) for img in running_image_data]
                if running_audio_data is not None:
                    extra_fields["audio"] = [encode_audio_data(audio) for audio in running_audio_data]
                ...
                tool_results["agent_prompt_length"] = list(stats_dict["agent_prompt_lengths"])
                tool_results["agent_output_length"] = list(stats_dict["agent_output_lengths"])
                # process observations and prepare for next turn
                obs_text = tool_results['obs']
                if prompt_state is not None:
                    closing_token_ids = list(getattr(self, "turn_end_token_ids", []) or [])
                    if closing_token_ids and running_prompt_ids[-len(closing_token_ids) :] != closing_token_ids:
                        running_prompt_ids.extend(closing_token_ids)
                        response_mask.extend([0] * len(closing_token_ids))
                        response_logprobs.extend([0.0] * len(closing_token_ids))
                    obs_token_ids = prompt_state.append_observation(obs_text, tool_results.get("image"))
                    running_image_data = list(prompt_state.running_image_data)
                ...
                if self.enable_mtrl and prompt_state is None:
        ...
        verl_tool_metrics = {
            ...
            "format_message_turns": len(stats_dict["format_messages"]),
            ...
            "agent_prompt_length_total": sum(stats_dict["agent_prompt_lengths"]),
            "agent_output_length_total": sum(stats_dict["agent_output_lengths"]),
        ...
        extra_fields = {
            ...
            "agent_prompt_lengths": list(stats_dict.get("agent_prompt_lengths", [])),
            "agent_output_lengths": list(stats_dict.get("agent_output_lengths", [])),
            "format_messages": list(stats_dict.get("format_messages", [])),
        }
        if prompt_state is not None:
            extra_fields["prompt_rebuild_payload"] = {
                "model_family": self.model_family,
                **prompt_state.build_rebuild_payload(),
            }
        if "uid" in kwargs:
            # Propagate uid for example-level masking downstream.
