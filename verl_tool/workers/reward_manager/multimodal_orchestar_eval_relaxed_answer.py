"""
Standalone copy-over eval-only relaxed-answer variant of the MultimodalOrchestra reward manager.

This module keeps the full reward-manager logic self-contained and only relaxes
answer extraction for evaluation:
- prefer <answer>...</answer>
- then fall back to \\boxed{...}
- then fall back to Answer:/Final Answer:
- finally, only when traj_stop_reason == model_chose_to_finish and the
  trajectory does not contain tool usage, use the final assistant text block
  as a conservative fallback answer

It also changes reward shaping so that:
...
class SampleInfo:
    ...
    answer_extraction_source: str = ""
    answer_rejection_reason: str = ""
...
LABELED_ANSWER_RE = re.compile(
    r"(?is)(?:\*\*)?\s*(?:final\s+answer|answer)\s*(?:\*\*)?\s*:\s*(.+?)(?=(?:\n\s*\n)|(?:\n(?:(?:\*\*)?\s*(?:final\s+answer|answer)\s*(?:\*\*)?\s*:))|\Z)"
)
ANSWER_INVALID_MARKERS = (
    "<tool_call>",
    "</tool_call>",
    "[Tool:",
    "<|im_start|>user",
...
@register("multimodal_orchestra_eval_relaxed_answer")
class MultimodalOrchestraRewardManager:
    ...
    name = "multimodal_orchestra_eval_relaxed_answer"
    ...
    def __init__(
        ...
        tool_penalty_start_step: int = 32,
        latency_penalty_start_step: int = 60,
        **kwargs,
    ):
    ...
    def _extract_sample_info(self, data_item: DataProto, index: int) -> Tuple[SampleInfo, Tuple[int, int]]:
        ...
        traj_stop_reason = str(data_item.non_tensor_batch.get("traj_stop_reason", "") or "")

        answer_extraction = self._extract_answer_result(
            response_str,
            traj_stop_reason=traj_stop_reason,
            tool_calls_info=tool_calls_info,
        )
        answer_text = answer_extraction["answer_text"]
        ...
        sample = SampleInfo(
            ...
            extracted_answer_text=str(answer_text),
            answer_extraction_source=str(answer_extraction["source"]),
            answer_rejection_reason=str(answer_extraction["rejection_reason"]),
    ...
    def _compute_reward(
        ...
        latency_penalty = 0.0
        if global_steps >= self.latency_penalty_start_step:
                ...
                entry["rollouts"].append(
                    {
                        ...
                        "format_reward": float(sample.format_reward),
                        "extracted_answer_text": str(sample.extracted_answer_text),
                        "answer_extraction_source": str(sample.answer_extraction_source),
                        "answer_rejection_reason": str(sample.answer_rejection_reason),
                        "tool_name": tool_name,
    ...
    def _extract_answer_text(
        ...
        tool_calls_info: Optional[ToolCallSummary] = None,
    ) -> str:
        return self._extract_answer_result(
            response_str,
            traj_stop_reason=traj_stop_reason,
            tool_calls_info=tool_calls_info,
        )["answer_text"]

    def _extract_answer_result(
        self,
        response_str: str,
        traj_stop_reason: str = "",
        tool_calls_info: Optional[ToolCallSummary] = None,
        ...
        if answer_match:
            return self._finalize_answer_candidate(answer_match.group(1).strip(), source="answer_tag")

        boxed_answer = self._extract_boxed_answer(response_str)
        if boxed_answer:
            return self._finalize_answer_candidate(boxed_answer, source="boxed_answer")

        labeled_answer = self._extract_labeled_answer(response_str)
        if labeled_answer:
            return self._finalize_answer_candidate(labeled_answer, source="labeled_answer")

        if self._is_tool_trajectory(response_str, tool_calls_info):
            return {
                "answer_text": "",
                "source": "tool_trajectory_requires_explicit_answer",
                "rejection_reason": "tool_trajectory_requires_explicit_answer",
            }

        if traj_stop_reason == "model_chose_to_finish":
            fallback_answer = self._extract_last_assistant_text_block(response_str)
            return self._finalize_answer_candidate(
                fallback_answer,
                source="finish_last_assistant_text",
                empty_reason="finish_tail_empty",
            )

        return {
            "answer_text": "",
            "source": "no_answer_extracted",
            "rejection_reason": f"traj_stop_reason={traj_stop_reason or 'missing'}",
        }
    ...
    def _finalize_answer_candidate(
        ...
        answer_text: str,
        source: str,
        empty_reason: str = "empty_candidate",
    ) -> Dict[str, str]:
        candidate = self._clean_answer_candidate(answer_text)
        if not candidate:
            return {"answer_text": "", "source": source, "rejection_reason": empty_reason}

        rejection_reason = self._answer_candidate_rejection_reason(candidate)
        if rejection_reason:
            return {"answer_text": "", "source": source, "rejection_reason": rejection_reason}

        return {"answer_text": candidate, "source": source, "rejection_reason": ""}

    def _extract_boxed_answer(self, response_str: str) -> str:
        matches = BOXED_ANSWER_RE.findall(response_str)
        if not matches:
            return ""
        return self._clean_answer_candidate(matches[-1][:-1])

    def _extract_labeled_answer(self, response_str: str) -> str:
        matches = LABELED_ANSWER_RE.findall(response_str)
        if not matches:
            return ""
        return self._clean_answer_candidate(matches[-1])

    def _extract_last_assistant_text_block(self, response_str: str) -> str:
        assistant_turns = re.findall(
            r"(?s)<\|im_start\|>assistant\s*(.*?)\s*<\|im_end\|>",
            str(response_str or ""),
        )
        for block in reversed(assistant_turns):
            candidate = self._clean_answer_candidate(block)
            if candidate:
                return candidate
    ...
    def _is_tool_trajectory(
        self,
        response_str: str,
        tool_calls_info: Optional[ToolCallSummary] = None,
    ) -> bool:
        if tool_calls_info is not None and bool(tool_calls_info.tool_calls):
            return True
        response_text = str(response_str or "")
        return any(marker in response_text for marker in TOOL_TRAJECTORY_MARKERS)

    def _answer_candidate_rejection_reason(self, answer_text: str) -> str:
        candidate = str(answer_text or "")
        for marker in ANSWER_INVALID_MARKERS:
            if marker in candidate:
                return f"candidate_contains_marker:{marker}"
        return ""

    def _clean_answer_candidate(self, text: Optional[str]) -> str:
        if not text:
            return ""
        candidate = str(text).strip()
        candidate = re.sub(r"(?is)^<(?:answer|thinking)>\s*", "", candidate)
        candidate = re.sub(r"(?is)\s*</(?:answer|thinking)>$", "", candidate)
        candidate = re.sub(r"(?is)^(?:\*\*)?\s*(?:final\s+answer|answer)\s*(?:\*\*)?\s*:\s*", "", candidate)
        candidate = candidate.strip().strip("*").strip()
        candidate = re.sub(r"\s*<\|im_end\|>\s*$", "", candidate)
        candidate = re.sub(r"\s*<\|endoftext\|>\s*$", "", candidate)
