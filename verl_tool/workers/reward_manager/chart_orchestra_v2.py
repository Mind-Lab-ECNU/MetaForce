...
Implements ToolOrchestra-style preferences with format reward checking.
"""
...
ANSWER_PATTERN = re.compile(
    r"^<thinking>(?:(?!<thinking>).)*?</thinking>\n{0,2}"
    r"<answer>(?:(?!<thinking>|<answer>).)*?</answer>$",
    re.DOTALL,
)
...
class ChartOrchestraRewardManager:
    ...
    - 格式奖励: 检查 TOOL_CALL_PATTERN 和 ANSWER_PATTERN 格式
    """

    name = "chart_orchestra_v2"
    ...
    def __call__(self, data: DataProto, return_dict: bool = False):
        ...
        for i, sample in enumerate(samples):
            total_reward, accuracy_reward, tool_reward, cost_reward, latency_reward, format_reward = (
            ...
            if already_printed.get(sample.data_source, 0) < self.num_examine:
                ...
                print(
                    ...
                    f"[format] {format_reward:.4f}, [raw_total] {total_reward:.4f}"
                )
    ...
    def _extract_sample_info(self, data_item: DataProto, index: int) -> Tuple[SampleInfo, Tuple[int, int]]:
        ...
        action_texts = list(data_item.non_tensor_batch.get("action_texts", []))
...
{{"correct": false}}"""
    ...
    def _compute_format_reward(self, action_texts: List[str]) -> float:
        """
        计算格式奖励:
        - 最后一轮必须匹配 ANSWER_PATTERN
        - 前面所有轮次必须匹配 TOOL_CALL_PATTERN
        - 全部正确返回 1.0，否则返回 -1.0
        """
        if not action_texts:
            return -1.0

        num_turns = len(action_texts)
        ...
        for i, text in enumerate(action_texts):
            ...
            if is_last:
                # 最后一轮用 ANSWER_PATTERN
                if not ANSWER_PATTERN.match(text):
                    return -1.0
            else:
                # 前面轮次用 TOOL_CALL_PATTERN
                if not TOOL_CALL_PATTERN.match(text):
                    return -1.0

        return 1.0

    def _compute_reward(self, sample: SampleInfo, batch_stats: BatchStats) -> Tuple[float, float, float, float, float, float]:
        uid = sample.uid
        ...
        # 计算格式奖励
        format_reward = self._compute_format_reward(sample.action_texts)
        ...
        if not sample.is_correct:
            return format_reward, accuracy_reward, 0.0, 0.0, 0.0, format_reward
        ...
        if latency_reward + cost_reward >= 0.8:
            total_reward = tool_total + accuracy_reward + format_reward - 0.2
        else:
            total_reward = tool_total + accuracy_reward + format_reward - (latency_reward + cost_reward) / 4
        return total_reward, accuracy_reward, tool_reward, cost_reward, latency_reward, format_reward
