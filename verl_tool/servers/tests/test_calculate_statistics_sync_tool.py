...
def test_calculate_statistics_model2(
    url: str = "http://localhost:5601/get_observation",
    image_path: str = IMAGE_PATH,
    question: str = "Calculate the median value from the chart.",
):
    img = _encode_image_as_data_url(image_path)
    action = f'<tool_call>{json.dumps({"name": "calculate_statistics", "arguments": {"statistics_type": "median", "model": "calculate_statistics-2"}})}</tool_call>'
    result = _send_request(url, action, img, question)
    _assert_success(result)
    return result


def test_with_return_info(
    url: str = "http://localhost:5601/get_observation",
    image_path: str = IMAGE_PATH,
    question: str = "Find the minimum value and return its label.",
):
    img = _encode_image_as_data_url(image_path)
    action = f'<tool_call>{json.dumps({"name": "calculate_statistics", "arguments": {"statistics_type": "min", "return_info": "label", "model": "calculate_statistics-1"}})}</tool_call>'
    result = _send_request(url, action, img, question)
    _assert_success(result)
    return result


def test_with_target_label(
    url: str = "http://localhost:5601/get_observation",
    image_path: str = IMAGE_PATH,
    question: str = "Calculate mean for the first data series.",
):
    img = _encode_image_as_data_url(image_path)
    action = f'<tool_call>{json.dumps({"name": "calculate_statistics", "arguments": {"statistics_type": "mean", "target_label": "Series 1", "model": "calculate_statistics-1"}})}</tool_call>'
    result = _send_request(url, action, img, question)
    _assert_success(result)
    return result
    ...
    fire.Fire(
        {
            ...
            "return_info": test_with_return_info,
            "target_label": test_with_target_label,
