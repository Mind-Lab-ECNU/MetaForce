...
# Multimodal Processor Tool Async 测试脚本
# 每个测试函数的输出会写入对应的日志文件
...
echo "=========================================="
echo "async-no-ray: multimodal_processor_tool"
echo "开始时间: $(date)"
echo "=========================================="
echo ""
...
${SERVER_PY} --tool_type multimodal_processor_tool --port "${PORT}" --workers_per_tool 8 > "${SERVER_LOG}" 2>&1 &
PID_ASYNC=$!
...
for test_info in "${TEST_FUNCTIONS[@]}"; do
    ...
    # 运行测试并保存输出到日志文件
    if python -m verl_tool.servers.tests.test_multimodal_processor_tool_async "${cmd}" --url "${BASE_URL}" > "${log_file}" 2>&1; then
