...
def test_env_image_accumulation(
    ...
    action5 = f'<tool_call>{json.dumps({"name": "Qwen3-VL-235B-A22B-Instruct", "arguments": {"prompt": "请描述图片", "image_index": 1}})}</tool_call>'
    payload5 = {
        "trajectory_ids": [trajectory_id],
        "actions": [action5],
        "extra_fields": [{}],
    ...
    action6 = f'<tool_call>{json.dumps({"name": "OpenCV", "arguments": {"operation": "grayscale", "image_index": 3}})}</tool_call>'
    payload6 = {
        "trajectory_ids": [trajectory_id],
        "actions": [action6],
        "extra_fields": [{}],
