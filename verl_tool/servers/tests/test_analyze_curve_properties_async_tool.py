...
def _load_models_and_pricing():
    ...
    model_mapping = {
        "analyze_curve_properties": {
            "analyze_curve_properties-1": chosen[0],
            "analyze_curve_properties-2": chosen[1],
        }
    }
...
def _send_request(url: str, action: str, image_data_url: str, question: str) -> dict:
    ...
    payload = {
        "trajectory_ids": ["curve-async-001"],
        "actions": [action],
        "extra_fields": [
            {
...
def test_analyze_curve_properties_model1(
    url: str = "http://localhost:5603/get_observation",
    image_path: str = IMAGE_PATH,
    question: str = "Analyze the smoothness of the curves in this chart.",
):
    img = _encode_image_as_data_url(image_path)
    action_payload = {
        "name": "analyze_curve_properties_async",
        "arguments": {
            "analysis_type": "smoothness",
            "model": "analyze_curve_properties-1"
        }
    }
...
def test_analyze_curve_properties_model2(
    url: str = "http://localhost:5603/get_observation",
    image_path: str = IMAGE_PATH,
    question: str = "Calculate the area under the curve.",
):
    img = _encode_image_as_data_url(image_path)
    action_payload = {
        "name": "analyze_curve_properties_async",
        "arguments": {
            "analysis_type": "area_under_curve",
            "model": "analyze_curve_properties-2"
        }
    }
    action = f'<tool_call>{json.dumps(action_payload)}</tool_call>'
    result = _send_request(url, action, img, question)
...
def main():
    fire.Fire(
        {
            "model1": test_analyze_curve_properties_model1,
            "model2": test_analyze_curve_properties_model2,
