...
# Multimodal Processor Tool Async 测试脚本（ID pool + skills + run_skill）

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
LOG_DIR="${SCRIPT_DIR}/logs_async"
SERVER_LOG="/tmp/tool_multimodal_processor_async_id.log"
SKILL_STORE_DIR="$(mktemp -d /tmp/verl_skills_async_id_XXXXXX)"
...
${SERVER_PY} --tool_type multimodal_processor_tool_adapt_skill_id --port "${PORT}" --workers_per_tool 8 > "${SERVER_LOG}" 2>&1 &
PID_ASYNC=$!
...
declare -a TEST_FUNCTIONS=(
    ...
    "python_code_rejected:test_async_python_code_rejected"
...
for test_info in "${TEST_FUNCTIONS[@]}"; do
    ...
    # if python -m verl_tool.servers.tests.test_multimodal_processor_tool_async_skill_id "${cmd}" --url "${BASE_URL}" > "${log_file}" 2>&1; then
    ...
    # echo ""
    echo -e "${YELLOW}[${func_name}]${NC} -> ${log_file}"
    if python -m verl_tool.servers.tests.test_multimodal_processor_tool_async_skill_id "${cmd}" --url "${BASE_URL}" > /dev/null 2>&1 ; then
        echo -e "${GREEN}PASSED${NC}"
    else
        echo -e "${RED}FAILED${NC}"
        FAIL_COUNT=$((FAIL_COUNT + 1))
