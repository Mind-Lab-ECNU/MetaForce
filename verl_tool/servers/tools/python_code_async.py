"""
Asynchronous version of python_code tool for execution in Firejail sandbox.
This version uses asyncio for concurrent execution.
...
def execute_python_sync(code: Union[str, List[str]], timeout: int = TIMEOUT, stdin: Optional[str] = None,
                        python_path: str = None, pre_import_lib: bool = False, 
                        use_firejail: bool = False) -> Tuple[str, str, bool, float]:
    """Execute Python code in a Firejail sandbox with a timeout. Returns (stdout, stderr, has_error, latency)."""
...
@register_tool
class PythonCodeAsyncTool(BaseTool):
    """
    Asynchronous Python code execution tool.
    ...
    def parse_action(self, action: str) -> Tuple[str, bool]:
        ...
        """
        print(f"[PythonCodeTool ASYNC] parse_action called, action preview: {action[:200]}")
        ...
        try:
            ...
            if "<tool_call>" in action and "</tool_call>" in action:
                ...
                print(f"[PythonCodeTool ASYNC] Extracted payload: {payload_str[:150]}")
                
                payload = json.loads(payload_str)
                print(f"[PythonCodeTool ASYNC] Parsed JSON: {payload}")
                ...
                if payload.get("name") == "python_code":
                    ...
                    if isinstance(arguments, dict) and "code" in arguments:
                        ...
                        print(f"[PythonCodeTool ASYNC] ✓ Successfully extracted code ({len(code)} chars)")
                        print(f"[PythonCodeTool ASYNC] Code preview: {code[:100]}")
                        return code, True
                else:
                    print(f"[PythonCodeTool ASYNC] Tool name mismatch: {payload.get('name')} != python_code")
            else:
                print(f"[PythonCodeTool ASYNC] No <tool_call> tags found in action")
                
        except json.JSONDecodeError as e:
            print(f"[PythonCodeTool ASYNC] JSON decode error: {e}")
        except Exception as e:
            print(f"[PythonCodeTool ASYNC] Unexpected error: {e}")
        
        print(f"[PythonCodeTool ASYNC] ✗ Returning invalid")
        return "", False

    async def conduct_action_async(self, trajectory_id, action, extra_field):
        """Execute the parsed action in a Firejail sandbox asynchronously."""
        print(f"[PythonCodeTool ASYNC] conduct_action_async called for trajectory: {trajectory_id}")
        
        parsed_code, is_valid = self.parse_action(action)
        
        if not is_valid:
            print(f"[PythonCodeTool ASYNC] Invalid action, returning error")
            observation = {
        ...
        print(f"[PythonCodeTool ASYNC] Executing code ({len(parsed_code)} chars)")
        ...
        print(f"[PythonCodeTool ASYNC] Execution complete, has_error: {has_error}")
        print(f"[PythonCodeTool ASYNC] obs preview: {obs_content[:200]}")
    ...
    def conduct_action(self, trajectory_id, action, extra_field):
        ...
        return asyncio.run(self.conduct_action_async(trajectory_id, action, extra_field))
