...
@register_tool
class ExtractChartDataAsyncTool(BaseTool):
    """
    异步版本：内部自带事件循环，不要再用 Ray 包装。

    期望 action:
        <tool_call>{"name": "extract_chart_data", "arguments": {...}}</tool_call>

    必填: chart_type, model
    选填: target_label/target_color/target_index，extra_field['images'] 或 arguments['image_path']
    """

    tool_type = "extract_chart_data_async"
    ...
    def get_usage_inst(self) -> str:
        return (
            "extract_chart_data_async: 提取图表数据（异步，勿用 Ray 包装）。\n"
    ...
    async def _conduct_action_async(
        ...
        try:
            ...
            print(
                f"[extract_chart_data_async] model_variant={model_variant}, service_name={meta.get('service_name')}, "
