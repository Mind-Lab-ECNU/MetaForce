...
@register_tool
class AnalyzeCurvePropertiesAsyncTool(BaseTool):
    """
    Asynchronous curve properties analysis tool.
    
    Expected action format:
        <tool_call>{"name": "analyze_curve_properties_async", "arguments": {"analysis_type": "smoothness", "model": "analyze_curve_properties-1"}}</tool_call>
    
    DO NOT wrap this tool with Ray - it manages its own event loop internally.
    """

    tool_type = "analyze_curve_properties_async"
    ...
    def get_usage_inst(self) -> str:
        return (
            "analyze_curve_properties_async: Analyze geometric and statistical properties of curves (async). "
            "Format: <tool_call>{\"name\": \"analyze_curve_properties_async\", "
    ...
    def parse_action(self, action: str) -> Tuple[Dict[str, Any], bool]:
        ...
        """
        print(f"[AnalyzeCurvePropertiesAsync] parse_action called, action preview: {action[:200]}")
        
        try:
            # Extract JSON from <tool_call> tags
            if "<tool_call>" not in action or "</tool_call>" not in action:
                print(f"[AnalyzeCurvePropertiesAsync] No <tool_call> tags found")
                return {}, False
            ...
            print(f"[AnalyzeCurvePropertiesAsync] Extracted payload: {payload_str[:150]}")
            
            payload = json.loads(payload_str)
            print(f"[AnalyzeCurvePropertiesAsync] Parsed JSON: {payload}")
            
            if payload.get("name") != "analyze_curve_properties_async":
                print(f"[AnalyzeCurvePropertiesAsync] Tool name mismatch: {payload.get('name')}")
                return {}, False
                
            arguments = payload.get("arguments", {})
            if not isinstance(arguments, dict):
                print(f"[AnalyzeCurvePropertiesAsync] Arguments not a dict")
                return {}, False
                
            if "analysis_type" not in arguments or "model" not in arguments:
                print(f"[AnalyzeCurvePropertiesAsync] Missing required fields")
                return {}, False
                
            payload["arguments"] = arguments
            print(f"[AnalyzeCurvePropertiesAsync] ✓ Successfully parsed action")
            return payload, True
            
        except json.JSONDecodeError as e:
            print(f"[AnalyzeCurvePropertiesAsync] JSON decode error: {e}")
            return {}, False
        except Exception as e:
            print(f"[AnalyzeCurvePropertiesAsync] Unexpected error: {e}")
            return {}, False
    ...
    async def _call_model_async(
        ...
        payload = {
            "model": service_name,
            "messages": [
                {"role": "system", "content": "You are a precise curve properties analysis assistant."},
                {
                    "role": "user",
                    "content": [
    ...
    async def _conduct_action_async(
        ...
        print(f"[AnalyzeCurvePropertiesAsync] _conduct_action_async called for trajectory: {trajectory_id}")
        
        parsed, is_valid = self.parse_action(action)
        if not is_valid:
            print(f"[AnalyzeCurvePropertiesAsync] Invalid action, returning error")
            observation = {
                "obs": "Invalid action format for analyze_curve_properties_async. Expected: <tool_call>{\"name\": \"analyze_curve_properties_async\", \"arguments\": {...}}</tool_call>",
        ...
        if not image_data_url:
            observation = {
                "obs": "No image provided for analyze_curve_properties_async.",
        ...
        try:
            ...
            print(
                f"[analyze_curve_properties_async] model_variant={model_variant}, service_name={meta.get('service_name')}, "
            ...
            observation = {
                "obs": f"analyze_curve_properties_async failed: {exc}",
