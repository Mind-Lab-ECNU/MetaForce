...
@register_tool
class CompareAndFilterTool(BaseTool):
    """
    Synchronous compare and filter tool (can be wrapped by Ray).
    
    Expected action format:
        <tool_call>{"name": "compare_and_filter", "arguments": {"comparison_type": "value_to_statistic", "condition": "greater", "model": "compare_and_filter-1"}}</tool_call>
    """

    tool_type = "compare_and_filter"
    ...
    def get_usage_inst(self) -> str:
        return (
            "compare_and_filter: Compare and filter data. "
            "Format: <tool_call>{\"name\": \"compare_and_filter\", "
    ...
    def parse_action(self, action: str) -> Tuple[Dict[str, Any], bool]:
        ...
        """
        print(f"[CompareAndFilter] parse_action called, action preview: {action[:200]}")
        
        try:
            # Extract JSON from <tool_call> tags
            if "<tool_call>" not in action or "</tool_call>" not in action:
                print(f"[CompareAndFilter] No <tool_call> tags found")
                return {}, False
            ...
            print(f"[CompareAndFilter] Extracted payload: {payload_str[:150]}")
            
            payload = json.loads(payload_str)
            print(f"[CompareAndFilter] Parsed JSON: {payload}")
            
            if payload.get("name") != "compare_and_filter":
                print(f"[CompareAndFilter] Tool name mismatch: {payload.get('name')}")
                return {}, False
                
            arguments = payload.get("arguments", {})
            if not isinstance(arguments, dict):
                print(f"[CompareAndFilter] Arguments not a dict")
                return {}, False
                
            if "comparison_type" not in arguments or "condition" not in arguments or "model" not in arguments:
                print(f"[CompareAndFilter] Missing required fields")
                return {}, False
                
            payload["arguments"] = arguments
            print(f"[CompareAndFilter] ✓ Successfully parsed action")
            return payload, True
            
        except json.JSONDecodeError as e:
            print(f"[CompareAndFilter] JSON decode error: {e}")
            return {}, False
        except Exception as e:
            print(f"[CompareAndFilter] Unexpected error: {e}")
            return {}, False
    ...
    def conduct_action(
        ...
        print(f"[CompareAndFilter] conduct_action called for trajectory: {trajectory_id}")
        
        parsed, is_valid = self.parse_action(action)
        if not is_valid:
            print(f"[CompareAndFilter] Invalid action, returning error")
            observation = {
                "obs": "Invalid action format for compare_and_filter. Expected: <tool_call>{\"name\": \"compare_and_filter\", \"arguments\": {...}}</tool_call>",
        ...
        if not image_data_url:
            observation = {
                "obs": "No image provided for compare_and_filter.",
        ...
        try:
            ...
            print(
                f"[compare_and_filter] model_variant={model_variant}, service_name={meta.get('service_name')}, "
            ...
            observation = {
                "obs": f"compare_and_filter failed: {exc}",
