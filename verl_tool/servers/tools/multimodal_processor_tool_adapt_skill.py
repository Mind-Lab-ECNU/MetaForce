...
class MultimodalProcessorTool(BaseTool):
    ...
    wolfram_tool_name = "WolframAlpha"
    default_wolfram_maxchars = 8192
    sam3_tool_name = "SAM3"
    ...
    unichart_tool_name = "UniChart"
    unichart_model_name = "UniChart"
    deplot_tool_name = "Deplot"
    deplot_model_name = "Deplot"
    chartmoe_tool_name = "ChartMoe"
    chartmoe_model_name = "ChartMoe"
    ...
    python_code_tool_name = "python_code"
    python_code_timeout = PYTHON_CODE_TIMEOUT
    python_code_pre_import_lib = True
    python_code_use_firejail = True
    python_code_python_path = None
    create_skill_tool_name = "create_skill"
    run_skill_tool_name = "run_skill"
    skill_gen_model_name = "Qwen3-VL-235B-A22B-Instruct-skill"
    ...
    def get_usage_inst(self) -> str:
        return (
            "multimodal_processor_tool_adapt_skill:\n"
            "Multimodal model call example (Qwen3-VL-8B/32B): <tool_call>{\"name\": \"Qwen3-VL-8B-Instruct\", "
            ...
            "\"arguments\": {\"prompt\": \"Describe the image\", \"image_index\": 1}}</tool_call>\n"
            "InternVL example: <tool_call>{\"name\": \"InternVL3.5-38B-Instruct\", "
            "\"arguments\": {\"prompt\": \"Describe the image\", \"image_index\": 1}}</tool_call>\n"
            "WolframAlpha example: <tool_call>{\"name\": \"WolframAlpha\", "
            "\"arguments\": {\"query\": \"10 densest elemental metals\"}}</tool_call>\n"
            ...
            "UniChart example: <tool_call>{\"name\": \"UniChart\", "
            "\"arguments\": {\"image_index\": 1, \"prompt\": \"<chartqa> What is the highest value? <s_answer>\"}}</tool_call>\n"
            "Deplot example: <tool_call>{\"name\": \"Deplot\", "
            "\"arguments\": {\"image_index\": 1, \"prompt\": \"Generate underlying data table of the figure below:\"}}</tool_call>\n"
            "ChartMoe example: <tool_call>{\"name\": \"ChartMoe\", "
            "\"arguments\": {\"image_index\": 1, \"prompt\": \"Describe and regenerate this chart with matplotlib.\"}}</tool_call>\n"
            "step3 example: <tool_call>{\"name\": \"step3\", "
            "\"arguments\": {\"prompt\": \"Describe the image\", \"image_index\": 1}}</tool_call>\n"
            ...
            "python_code example: <tool_call>{\"name\": \"python_code\", "
            "\"arguments\": {\"code\": \"print('hello')\"}}</tool_call>\n"
    ...
    async def _call_wolfram_alpha_async(
        self,
        query: str,
    ) -> Tuple[str, Dict[str, Any]]:
        base_url = self.url_mapping.get(self.wolfram_tool_name)
        if not base_url:
            raise ValueError(f"Model URL for {self.wolfram_tool_name} not found.")
        ...
        def _post():
            ...
            resp.raise_for_status()
            data = resp.json()
            return data.get("data", {}).get("result", ""), latency

        text, latency = await loop.run_in_executor(None, _post)
        metadata = {
            "latency": latency,
            "model": self.wolfram_tool_name,
        }
    ...
    async def _call_unichart_async(
        self,
        image_base64: str,
        prompt: Optional[str] = None,
    ) -> Tuple[Dict[str, Any], Dict[str, Any]]:
        base_url = self.url_mapping.get(self.unichart_model_name)
        if not base_url:
            raise ValueError(f"Model URL for {self.unichart_model_name} not found.")
        ...
        metadata = {
            "latency": latency,
            "model": self.unichart_model_name,
        }
        return data, metadata

    async def _call_deplot_async(
        self,
        image_base64: str,
        prompt: Optional[str] = None,
    ) -> Tuple[Dict[str, Any], Dict[str, Any]]:
        base_url = self.url_mapping.get(self.deplot_model_name)
        if not base_url:
            raise ValueError(f"Model URL for {self.deplot_model_name} not found.")
        ...
        metadata = {
            "latency": latency,
            "model": self.deplot_model_name,
        }
        return data, metadata

    async def _call_chartmoe_async(
        self,
        image_base64: str,
        prompt: Optional[str] = None,
    ) -> Tuple[Dict[str, Any], Dict[str, Any]]:
        base_url = self.url_mapping.get(self.chartmoe_model_name)
        if not base_url:
            raise ValueError(f"Model URL for {self.chartmoe_model_name} not found.")
        ...
        metadata = {
            "latency": latency,
            "model": self.chartmoe_model_name,
        }
        return data, metadata
    ...
    async def _conduct_action_async(
        ...
        if tool_name == self.wolfram_tool_name:
            query = arguments.get("query") or arguments.get("input")
            if query is None:
                observation = {
                    "obs": "Missing parameters: query is required.",
                    "invalid_reason": "missing_parameters",
                }
                return _finalize(observation, False, False)

            try:
                text, meta = await self._call_wolfram_alpha_async(str(query))
                observation = {
                    "obs": f"[Tool: {self.wolfram_tool_name}] {text}",
                    "tool": self.wolfram_tool_name,
                    "model": self.wolfram_tool_name,
        ...
        if tool_name == self.python_code_tool_name:
            code = _extract_python_code_argument(arguments)
            if not code:
                observation = {
                    "obs": "Missing parameters: code is required.",
                    "invalid_reason": "missing_parameters",
                }
                return _finalize(observation, False, False)

            stdin = arguments.get("stdin") if isinstance(arguments, dict) else None
            if stdin is None and isinstance(extra_field, dict):
                stdin = extra_field.get("stdin")
            ...
            loop = asyncio.get_event_loop()
            try:
                stdout, stderr, has_error, latency = await loop.run_in_executor(
                    None,
                    _execute_python_code_sync,
                    code,
                    self.python_code_timeout,
                    stdin,
                    self.python_code_python_path,
                    self.python_code_pre_import_lib,
                    self.python_code_use_firejail,
                )
            except Exception as exc:
                observation = {
                    "obs": f"multimodal_processor_tool failed: {exc}",
            ...
            obs_content = execution_result[:PYTHON_CODE_MAX_OBS_LENGTH]

            observation = {
                "obs": obs_content,
                "tool": self.python_code_tool_name,
                "model": self.python_code_tool_name,
                "latency": latency,
                "cost": 0.0,
                "usage": {},
                "has_error": has_error,
        ...
        if tool_name == self.unichart_tool_name:
            image_index = arguments.get("image_index")
            if image_index is None:
                observation = {
                    "obs": "Missing parameters: image_index is required.",
            ...
            try:
                result, meta = await self._call_unichart_async(
                    image_base64=image_base64,
            ...
            observation = {
                "obs": f"[Tool: {self.unichart_tool_name}] {json.dumps(result, ensure_ascii=False)}",
                "tool": self.unichart_tool_name,
                "model": self.unichart_model_name,
                "latency": meta.get("latency"),
            }
            return _finalize(observation, False, True)

        if tool_name == self.deplot_tool_name:
            image_index = arguments.get("image_index")
            if image_index is None:
                observation = {
                    "obs": "Missing parameters: image_index is required.",
            ...
            try:
                result, meta = await self._call_deplot_async(
                    image_base64=image_base64,
                    prompt=str(prompt) if prompt is not None else None,
                )
            except ValueError as exc:
            ...
            observation = {
                "obs": f"[Tool: {self.deplot_tool_name}] {json.dumps(result, ensure_ascii=False)}",
                "tool": self.deplot_tool_name,
                "model": self.deplot_model_name,
                "latency": meta.get("latency"),
            }
            return _finalize(observation, False, True)

        if tool_name == self.chartmoe_tool_name:
            image_index = arguments.get("image_index")
            if image_index is None:
                observation = {
                    "obs": "Missing parameters: image_index is required.",
            ...
            try:
                result, meta = await self._call_chartmoe_async(
                    image_base64=image_base64,
                    prompt=str(prompt) if prompt is not None else None,
                )
            except ValueError as exc:
            ...
            observation = {
                "obs": f"[Tool: {self.chartmoe_tool_name}] {json.dumps(result, ensure_ascii=False)}",
                "tool": self.chartmoe_tool_name,
                "model": self.chartmoe_model_name,
                "latency": meta.get("latency"),
            }
            return _finalize(observation, False, True)
