#!/usr/bin/env python3
"""Update prompts for test_merge_v2 ID/OOD datasets and rewrite parquet/json."""
...
def _find_test_files(data_root: Path) -> List[Path]:
    files = []
    files.extend(sorted((data_root / "ID").glob("*/test.json")))
    files.extend(sorted((data_root / "OOD").glob("*/test.json")))
    return files
...
def main() -> None:
    ...
    if not files:
        print("No test_merge_v2 files found.")
        return

    print("=" * 80)
    print("Update prompts for test_merge_v2")
    print("=" * 80)
