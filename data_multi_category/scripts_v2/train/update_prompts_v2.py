#!/usr/bin/env python3
"""Update prompts for train_merge_v2 datasets and rewrite parquet/json."""
...
def _find_train_files(data_root: Path) -> List[Path]:
    return sorted(data_root.glob("*/train.json"))
...
def main() -> None:
    ...
    files = _find_train_files(train_merge_root)
    if not files:
        print("No train_merge_v2 files found.")
        return

    print("=" * 80)
    print("Update prompts for train_merge_v2")
    print("=" * 80)
    ...
    for path in files:
        ...
        processed = [ensure_extra_info_jsonable(s) for s in valid]
        ...
        print(
            f"{path}: raw={len(raw)} updated={len(processed)} dropped={len(errors)}"
        )

    print("-" * 80)
    print(f"Updated train samples: {total}")
