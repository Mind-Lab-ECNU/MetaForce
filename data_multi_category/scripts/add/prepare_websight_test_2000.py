...
"""
Preprocess the WebSight dataset to parquet format with local images.
...
def _extract_sample(example: dict) -> tuple[str, str, list, str, dict]:
    question = _pick_text(example.get("question"), default="Describe the webpage shown in the image.")
    answer = example.get("description") or example.get("text")
    image_item = example.get("image") or example.get("image_url")
    html = example.get("html")
    image_items = [image_item] if image_item is not None else []
    return question, _normalize_answer(answer), image_items, "", {"html": html}


def main() -> None:
    parser = argparse.ArgumentParser()
    parser.add_argument("--dataset_path", default="haidark1/WebSightDescribed")
    parser.add_argument("--dataset_config", default=None)
    parser.add_argument("--output_dir", default="/inspire/hdd/project/ai4education/public/wsa_1.0/verltools/verl_m/data_multi_category/data/add/code")
