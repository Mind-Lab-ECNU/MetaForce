...
"""
Preprocess the TextCaps dataset to parquet format with local images.
...
def _extract_sample(example: dict) -> tuple[str, str, list, str, dict]:
    question = _pick_text(example.get("question"), default="Describe the image.")
    answer = example.get("caption_str") or example.get("reference_strs") or example.get("caption")
...
def main() -> None:
    ...
    parser.add_argument("--dataset_path", default="lmms-lab/TextCaps")
