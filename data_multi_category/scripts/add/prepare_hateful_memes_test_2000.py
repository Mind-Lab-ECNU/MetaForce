...
"""
Preprocess the HatefulMemes dataset to parquet format with local images.
...
def _collect_base_dirs(split_dataset) -> list[Path]:
    base_dirs = []
    try:
        cache_files = split_dataset.cache_files
    except Exception:
        return base_dirs
...
def _copy_or_save_image(
    image_item, dest_dir: Path, name_prefix: str, base_dirs=None, hf_image_base: Optional[str] = None
) -> str:
    ...
    if isinstance(image_item, str):
        ...
        if hf_image_base:
            ext = _safe_ext(image_item)
            dest_path = dest_dir / f"{name_prefix}{ext}"
            url = urllib.parse.urljoin(hf_image_base, image_item)
    ...
    if isinstance(image_item, dict):
        if "path" in image_item and image_item["path"]:
            ...
            if src_path.exists():
                ...
                return dest_path.as_posix()
            if hf_image_base:
                ext = _safe_ext(image_item["path"])
                dest_path = dest_dir / f"{name_prefix}{ext}"
                url = urllib.parse.urljoin(hf_image_base, image_item["path"])
                _download_url(url, dest_path)
                return dest_path.as_posix()
...
def _extract_sample(example: dict) -> tuple[str, str, list, str, dict]:
    text = _pick_text(example.get("text"), default="")
    question = "Is this meme hateful?"
    if text:
        question = f"{question} Meme text: {text}"

    label = example.get("label")
    if isinstance(label, str):
        label_value = label.strip().lower()
        answer = "hateful" if label_value in {"1", "true", "yes", "hateful"} else "non-hateful"
    elif label is None:
        answer = ""
    else:
        answer = "hateful" if int(label) == 1 else "non-hateful"

    image_item = example.get("img") or example.get("image") or example.get("image_url")
    image_items = [image_item] if image_item is not None else []
    return question, _normalize_answer(answer), image_items, "", {"text": text}


def main() -> None:
    parser = argparse.ArgumentParser()
    parser.add_argument("--dataset_path", default="neuralcatcher/hateful_memes")
    ...
    parser.add_argument("--hf_image_base", default=None, help="Optional base URL for HF image files.")
    ...
    dataset_name = "HatefulMemes_2000"
    ...
    hf_image_base = args.hf_image_base
    if not hf_image_base and "/" in args.dataset_path and not Path(args.dataset_path).exists():
        hf_image_base = f"https://huggingface.co/datasets/{args.dataset_path}/resolve/main/"
    ...
    for split_name, split_dataset in dataset.items():
        ...
        base_dirs = _collect_base_dirs(split_dataset)
        ...
        for local_idx, orig_idx in enumerate(sampled_indices):
            ...
            for img_idx, image_item in enumerate(image_items):
                name_prefix = f"{split_name}_{local_idx}_{img_idx}"
                try:
