...
"""
Preprocess the TallyQA dataset to parquet format with local images.
...
def _extract_sample(example: dict, qa_item=None) -> tuple[str, str, list, str, dict]:
    question = None
    answer = None
    extra_fields = {}

    if isinstance(qa_item, dict):
        question = qa_item.get("question") or qa_item.get("question_text") or qa_item.get("query")
        answer = qa_item.get("answer") or qa_item.get("answers") or qa_item.get("label")
        if "is_simple" in qa_item:
            extra_fields["is_simple"] = qa_item.get("is_simple")
        if "data_source" in qa_item:
            extra_fields["data_source"] = qa_item.get("data_source")

    if question is None:
        question = example.get("question") or example.get("prompt")
    if answer is None:
        answer = example.get("answer") or example.get("completion") or example.get("label")

    question = _pick_text(question, default="")

    images = example.get("image") or example.get("images") or example.get("img")
    if images is None:
        image_items = []
    ...
    return question, _normalize_answer(answer), image_items, "", extra_fields


def main() -> None:
    parser = argparse.ArgumentParser()
    parser.add_argument("--dataset_path", default="vikhyatk/tallyqa-test")
    ...
    for split_name, split_dataset in dataset.items():
        ...
        for local_idx, orig_idx in enumerate(sampled_indices):
            ...
            qa_item = None
            qa_list = example.get("qa")
            if isinstance(qa_list, list) and qa_list:
                qa_item = random.choice(qa_list)
            try:
                question_text, answer, image_items, context_text, extra_fields = _extract_sample(
                    example, qa_item=qa_item
                )
            except Exception as e:
