...
"""
Preprocess the VQAv2 dataset to parquet format with local images.
...
def _extract_sample(example: dict) -> tuple[str, str, list, str, dict]:
    question = _pick_text(example.get("question") or example.get("question_text"), default="")
    answer = (
        example.get("multiple_choice_answer")
...
def _load_dataset_with_fallback(
    dataset_path: str, dataset_config: str | None
...
def main() -> None:
    ...
    parser.add_argument("--dataset_path", default="HuggingFaceM4/VQAv2")
    ...
    try:
        dataset = _load_dataset_with_fallback(args.dataset_path, args.dataset_config)
    except Exception as e:
        print(f"Error loading dataset {args.dataset_path} ({args.dataset_config}): {e}")
        return

    dataset_name = "VQAv2_2000"
