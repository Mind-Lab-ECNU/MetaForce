...
"""
Preprocess the OCR-VQA dataset to parquet format with local images.
...
def _extract_sample(example: dict) -> tuple[str, str, list, str, dict]:
    ...
    if isinstance(questions, list) and questions:
        ...
        if isinstance(answers, list) and answers:
            ans = answers[idx] if idx < len(answers) else answers[0]
        else:
            ans = answers
    else:
        question = _pick_text(example.get("question"), default="")
        ans = example.get("answer") or answers

    image_item = example.get("image")
    if image_item is None:
        image_item = example.get("image_url")

    image_items = [image_item] if image_item is not None else []
    return question, _normalize_answer(ans), image_items, "", {}


def main() -> None:
    parser = argparse.ArgumentParser()
    parser.add_argument("--dataset_path", default="howard-hou/OCR-VQA")
    parser.add_argument("--dataset_config", default=None)
    parser.add_argument("--output_dir", default="/inspire/hdd/project/ai4education/public/wsa_1.0/verltools/verl_m/data_multi_category/data/add/ocr")
