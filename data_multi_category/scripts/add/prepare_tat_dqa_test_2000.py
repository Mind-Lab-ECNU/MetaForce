...
"""
Preprocess the TAT-DQA test dataset to parquet format with local images.
...
def _extract_sample(example: dict) -> tuple[str, str, list, str, dict]:
    ...
    if image_item is None:
        image_item = example.get("image_filename")

    image_items = [image_item] if image_item is not None else []

    extra_fields = {}
    for key in ("questionId", "docId", "page", "image_filename", "data_split", "source"):
        if key in example:
            extra_fields[key] = example.get(key)
...
def main() -> None:
    ...
    parser.add_argument("--dataset_path", default="vidore/tatdqa_test")
    parser.add_argument("--dataset_config", default=None)
    parser.add_argument("--split", default="test")
    parser.add_argument("--output_dir", default="/inspire/hdd/project/ai4education/public/wsa_1.0/verltools/verl_m/data_multi_category/data/add/table")
    ...
    try:
        if args.dataset_config is None:
            split_dataset = datasets.load_dataset(args.dataset_path, split=args.split)
        else:
            split_dataset = datasets.load_dataset(args.dataset_path, args.dataset_config, split=args.split)
        dataset = {args.split: split_dataset}
    except Exception as e:
        print(f"Error loading dataset {args.dataset_path} ({args.dataset_config}) split={args.split}: {e}")
        return

    dataset_name = "TATDQA_2000"
