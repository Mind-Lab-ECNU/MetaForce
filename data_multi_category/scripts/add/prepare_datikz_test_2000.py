...
"""
Preprocess the DatiKZ dataset to parquet format with local images.
...
def _extract_sample(example: dict) -> tuple[str, str, list, str, dict]:
    question = _pick_text(example.get("question"), default="Convert the diagram to TikZ code.")
    answer = (
        example.get("tikz")
        or example.get("code")
        or example.get("latex")
        or example.get("target")
        or example.get("caption")
        or example.get("text")
    )

    image_item = example.get("image") or example.get("img") or example.get("image_url")
...
def main() -> None:
    ...
    parser.add_argument("--dataset_path", default="nllg/datikz")
    parser.add_argument("--dataset_config", default=None)
    parser.add_argument("--output_dir", default="/inspire/hdd/project/ai4education/public/wsa_1.0/verltools/verl_m/data_multi_category/data/add/diagram")
