...
"""
Preprocess the DocVQA dataset to parquet format with local images.
...
def main() -> None:
    ...
    parser.add_argument("--dataset_path", default="lmms-lab/DocVQA")
    parser.add_argument("--output_dir", default="/inspire/hdd/project/ai4education/public/wsa_1.0/verltools/verl_m/data_multi_category/data/doc")
    ...
    try:
        dataset = datasets.load_dataset(args.dataset_path, "DocVQA")
    ...
    for split_name, split_dataset in dataset.items():
        ...
        for local_idx, orig_idx in enumerate(sampled_indices):
            ...
            try:
                # Field mapping: questionId, question, image, docId, ucsf_document_id, ucsf_document_page_no, answers
                question_id = example["questionId"]
                question = example["question"]
                image_item = example["image"]
                answers = example["answers"]
            except KeyError as e:
                print(f"Skipping example {orig_idx} due to missing field: {e}")
                continue

            if isinstance(answers, list):
                if not answers:
                    print(f"Skipping example {orig_idx} due to empty answers")
                    continue
            ...
            mm_content = _build_mm_content(question, args.image_sep, len(local_image_paths))
            ...
            # Unused fields: answers: original answers list (string array), docId: document unique ID (int), ucsf_document_id: UCSF document ID (string), ucsf_document_page_no: document page number (string)
            data = {
