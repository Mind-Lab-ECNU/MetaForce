...
"""
Preprocess the PlotQA test dataset (jinaai/plotqa) to parquet format with local images.
...
def _extract_plotqa_fields(example: dict) -> tuple[str, str, object]:
    ...
    image_item, _ = _first_value(
        example,
        ["image", "images", "img", "chart", "image_path", "image_url", "image_url_path"],
    )
...
def main() -> None:
    ...
    parser.add_argument("--dataset_path", default="jinaai/plotqa")
    parser.add_argument(
        "--output_dir",
    ...
    for split_name, split_dataset in dataset.items():
        ...
        for local_idx, orig_idx in enumerate(sampled_indices):
            ...
            try:
                question_text, answer, image_item = _extract_plotqa_fields(example)
            except Exception as e:
            ...
            extra_info = {
                ...
                "question_id": example.get("question_id") or example.get("qid"),
