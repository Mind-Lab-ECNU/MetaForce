...
"""
Preprocess the FigureQA no_annot_test2 dataset (raw data) to parquet format with local images.
Uniformly sample 2000 examples from the test split.
Only examples with answers are saved.
"""
...
def main() -> None:
    ...
    parser.add_argument(
        "--raw_root",
        default="/inspire/hdd/project/ai4education/public/wsa_1.0/verltools/verl_m/data_multi_category/scripts/raw_data/FigureQA/no_annot_test2",
    )
    parser.add_argument(
        "--output_dir",
    ...
    if not items:
        print("No labeled items collected. Skipping save.")
        return
