...
"""
Preprocess the AI2D no-mask test dataset (lmms-lab/ai2d-no-mask) to parquet format with local images.
...
def _normalize_answer(value) -> str | None:
    ...
    if isinstance(value, list):
        ...
        if isinstance(first, dict):
            for k in ("answer", "value", "text", "label"):
...
def _extract_ai2d_fields(example: dict) -> tuple[str, str, object]:
    ...
    answer, _ = _first_value(
        example,
        ["answer", "answers", "label", "target", "final_answer", "correct_answer"],
    )
    image_item, _ = _first_value(
        example,
        ["image", "images", "img", "image_path", "image_url", "image_url_path"],
    )
    ...
    return question_text, answer_text, image_item
...
def _select_split(dataset_obj):
    ...
    raise KeyError(f"Could not find test split in dataset. Available: {list(dataset_obj.keys())}")


def main() -> None:
    parser = argparse.ArgumentParser()
    parser.add_argument("--dataset_path", default="lmms-lab/ai2d-no-mask")
    parser.add_argument(
        "--output_dir",
    ...
    for local_idx, orig_idx in enumerate(sampled_indices):
        ...
        try:
            question_text, answer, image_item = _extract_ai2d_fields(example)
        except Exception as e:
