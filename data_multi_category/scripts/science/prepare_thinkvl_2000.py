...
"""
Preprocess the ImageNet-Think dataset to parquet format with local images.
...
def _collect_answers(example: dict) -> list[str]:
    answers = []
    if example.get("answer_1") is not None:
        text = str(example["answer_1"]).strip()
        if text:
            answers.append(text)
    if example.get("answer_2") is not None:
        text = str(example["answer_2"]).strip()
        if text:
            answers.append(text)
    if not answers:
        raise ValueError("Missing required fields: answer_1 or answer_2.")
    return answers


def main() -> None:
    parser = argparse.ArgumentParser()
    parser.add_argument("--dataset_path", default="krishnateja95/ImageNet-Think")
    parser.add_argument("--output_dir", default="/inspire/hdd/project/ai4education/public/wsa_1.0/verltools/verl_m/data_multi_category/data/science")
    ...
    dataset_name = "ImageNetThink_2000"
    ...
    for split_name, split_dataset in dataset.items():
        ...
        for local_idx, orig_idx in enumerate(sampled_indices):
            ...
            if "image" not in example or "question" not in example:
                print(f"Skipping example {orig_idx}: missing required fields 'image'/'question'")
                continue

            try:
                answers = _collect_answers(example)
            except Exception as e:
            ...
            if image_item is None:
                print(f"Skipping example {orig_idx}: missing image")
                continue

            local_image_paths = []
            try:
                local_path = _copy_or_save_image(image_item, split_image_dir, f"{split_name}_{local_idx}_0")
            ...
            extra_info = {
                ...
                "think_1": example.get("think_1"),
                "answer_1": example.get("answer_1"),
                "think_2": example.get("think_2"),
                "answer_2": example.get("answer_2"),
            }
