...
"""
Preprocess the ScienceQA dataset to parquet format with local images.
...
def _extract_scienceqa_qa(example: dict) -> tuple[str, str, str | None]:
    """Extract question, answer, and formatted choices for ScienceQA-like schemas."""
...
def _has_non_empty_answer(example: dict) -> bool:
    answer = example.get("answer")
    if answer is None:
        answer = example.get("label")
    if answer is None:
...
def main() -> None:
    ...
    parser.add_argument("--dataset_path", default="derek-thomas/ScienceQA")
    ...
    for split_name, split_dataset in dataset.items():
        ...
        # Filter out empty ground-truth answers before sampling.
        valid_indices = [
            idx
            for idx in range(len(split_dataset))
            if _has_non_empty_answer(split_dataset[idx])
        ]

        # Uniform sampling
        n_samples = min(args.sample_count, len(valid_indices)) if args.sample_count else len(valid_indices)
        sampled_indices = random.sample(valid_indices, n_samples)
        sampled_indices = sorted(sampled_indices)

        for local_idx, orig_idx in enumerate(sampled_indices):
            example = split_dataset[orig_idx]
            try:
                question_text, answer, _ = _extract_scienceqa_qa(example)
            except Exception as e:
