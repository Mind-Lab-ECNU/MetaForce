...
"""
Preprocess the VizWiz dataset to parquet format with local images.
...
def _normalize_vizwiz_answer(answers) -> str | None:
    if answers is None:
        return None
...
def main() -> None:
    ...
    parser.add_argument("--dataset_path", default="lmms-lab/VizWiz-VQA")
    ...
    for split_name, split_dataset in dataset.items():
        ...
        for local_idx, orig_idx in enumerate(sampled_indices):
            ...
            if "question" not in example or "image" not in example:
                print(f"Skipping example {orig_idx} due to missing required fields.")
                print(f"Available keys: {list(example.keys())}")
                continue

            question_text = str(example["question"])
            answer = _normalize_vizwiz_answer(example.get("answers"))
            if not answer:
                print(f"Skipping example {orig_idx} due to missing answers.")
                continue

            image_item = example.get("image")
            if image_item is None:
                print(f"Skipping example {orig_idx} due to missing image.")
                continue
