#!/usr/bin/env python3
"""Build CLEVR_MATH OOD test_merge_v4 datasets from existing processed files."""
...
OUTPUT_NAME = "CLEVR_MATH_test_OOD"
...
def main() -> None:
    ...
    print("=" * 80)
    print("Prepare CLEVR_MATH OOD test_merge_v4 (pure reuse)")
    print("=" * 80)
    ...
    for subset in SUBSETS:
        ...
        reused = stable_sample(valid, args.sample_per_subset, args.seed)
        for sample in reused:
            sample["data_source"] = OUTPUT_NAME
        ...
        print(
            f"subset={subset}: raw={len(raw)} valid={len(valid)} selected={len(reused)} invalid={len(errors)}"
        )
