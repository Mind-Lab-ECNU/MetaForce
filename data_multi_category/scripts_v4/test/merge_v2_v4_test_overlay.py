#!/usr/bin/env python3
"""Overlay v4 test datasets onto v2 datasets by data_source, split-aware.

Example:
    python /inspire/hdd/project/ai4education/public/wsa_1.0/verltools/verl_m/data_multi_category/scripts_v4/test/merge_v2_v4_test_overlay.py \
      ...
      --v4-test-merge-dir-name /inspire/hdd/project/ai4education/public/wsa_1.0/verltools/verl_m/data_multi_category/data/test_merge_v4_clevr_math_tool_id \
      --out-test-merge-dir-name test_merge_v8_tool_id \
      --strict
"""
...
def _overlay_dataset_paths(v2_group_dir: Path, v4_group_dir: Path) -> Tuple[Dict[str, Path], List[str]]:
    v2_paths = _collect_dataset_files(v2_group_dir)
    v4_paths = _collect_dataset_files(v4_group_dir)

    merged = dict(v2_paths)
    overridden: List[str] = []
    for dataset_name, path in v4_paths.items():
...
def main() -> None:
    ...
    parser.add_argument("--v4-test-merge-dir-name", type=str, default="test_merge_v4")
    parser.add_argument("--out-test-merge-dir-name", type=str, default="test_merge_v2_overlay_v4")
    ...
    v4_root = output_base_dir / args.v4_test_merge_dir_name
    ...
    print(
        ...
        f"overlay={args.v4_test_merge_dir_name}, output={args.out_test_merge_dir_name}"
    )
    print("=" * 80)

    id_paths, overridden_id = _overlay_dataset_paths(v2_root / "ID", v4_root / "ID")
    ood_paths, overridden_ood = _overlay_dataset_paths(v2_root / "OOD", v4_root / "OOD")
    ...
    out_test_id_json = (
        ...
        else out_root / "test_id_merge_v2_overlay_v4.json"
    )
    out_test_ood_json = (
        args.out_test_ood_json
        if args.out_test_ood_json is not None
        else out_root / "test_ood_merge_v2_overlay_v4.json"
    )
    out_test_all_json = (
        args.out_test_all_json
        if args.out_test_all_json is not None
        else out_root / "test_merge_v2_overlay_v4.json"
    )
