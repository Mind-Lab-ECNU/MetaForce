#!/usr/bin/env python3
"""Common helpers for scripts_v4 pure-reuse OOD test processing."""
...
def load_tools(script_dir: Path, tool_json: str) -> str:
    tool_path = Path(tool_json)
    if not tool_path.is_absolute():
        tool_path = script_dir / tool_json
    if not tool_path.exists():
        raise FileNotFoundError(f"Tool JSON not found: {tool_path}")
...
def normalize_record_for_reuse(sample: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not isinstance(item.get("reward_model"), dict):
...
def resolve_image_path(image_path: str, data_root: Path) -> Path:
    path = Path(remap_image_path(image_path))
    if path.is_absolute():
        return path
...
def standardize_prompt_fields(sample: Dict[str, Any], system_prompt: str) -> Dict[str, Any]:
    ...
    item["images"] = [{"image": p} for p in image_paths]
    item["extra_info"] = extra_info
    item["reward_model"] = {"style": "rule", "ground_truth": get_ground_truth(item)}
...
def _normalize_list(value: Any, item_type: str) -> Any:
    ...
    if item_type == "number":
        ...
        return out
    if item_type == "bool":
        return [item if isinstance(item, bool) else bool(item) for item in values]
    if item_type == "string":
...
def validate_sample_for_merge(sample: Dict[str, Any], data_root: Path) -> List[str]:
    ...
    for path in image_paths + extra_images:
        resolved = resolve_image_path(path, data_root)
        if not is_image_readable(resolved):
            errors.append(f"image path unreadable: {path}")
