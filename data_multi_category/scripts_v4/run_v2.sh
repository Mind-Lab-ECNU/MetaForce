...
TEST_MERGE_DIR_NAME="${TEST_MERGE_DIR_NAME:-test_merge_v4}"
...
OUT_TEST_OOD_JSON="${OUT_TEST_OOD_JSON:-${OUTPUT_BASE_DIR}/${TEST_MERGE_DIR_NAME}/test_ood_merge_v4.json}"
...
python "${SCRIPT_DIR}/test/prepare_reuse_ood_test_merge_v4.py" \
  "${COMMON_PATH_ARGS[@]}" \
  --seed "${SEED}" \
  --sample-per-subset "${SAMPLE_PER_SUBSET}"

echo "==> [2/3] update test prompts"
python "${SCRIPT_DIR}/test/update_prompts_test_v4.py" \
  "${COMMON_PATH_ARGS[@]}" \
  --tool-json "${TEST_TOOL_JSON}" \
  "${STRICT_ARGS[@]}"

echo "==> [3/3] merge test"
python "${SCRIPT_DIR}/test/merge_datasets_to_parquet_test_v4.py" \
  "${COMMON_PATH_ARGS[@]}" \
