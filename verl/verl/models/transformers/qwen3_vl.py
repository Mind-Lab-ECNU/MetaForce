...
# 正确的类名
if hasattr(qwen3_vl_module, 'Qwen3VLVisionModel'):
    VisionModelClass = qwen3_vl_module.Qwen3VLVisionModel
    
    # 保存原始函数
    _original_fast_pos_embed_interpolate = VisionModelClass.fast_pos_embed_interpolate
    ...
    def _patched_fast_pos_embed_interpolate(self, grid_thw):
        ...
        # 确保 grid_thw 在正确设备上
        if grid_thw is not None and grid_thw.device != target_device:
            print(f"[DEVICE FIX] Moving grid_thw from {grid_thw.device} to {target_device}")
            grid_thw = grid_thw.to(target_device)
        
        # 临时修改 torch 的默认设备来确保内部创建的张量也在正确设备上
        import torch
        
        # 方法1: 使用 device context (PyTorch 2.0+)
        try:
    ...
    # 应用 patch
    VisionModelClass.fast_pos_embed_interpolate = _patched_fast_pos_embed_interpolate
    print("[INFO] ✓ Applied device fix patch to Qwen3VLVisionModel")
...
def _get_input_embeds(
    ...
    video_grid_thw: Optional[torch.LongTensor] = None,
     ...
     # ========== 添加调试信息 ==========
    print(f"[DEBUG] _get_input_embeds called")
    print(f"[DEBUG] input_ids device: {input_ids.device}")
    if image_grid_thw is not None:
        print(f"[DEBUG] image_grid_thw BEFORE transfer - device: {image_grid_thw.device}, dtype: {image_grid_thw.dtype}, shape: {image_grid_thw.shape}")
    if video_grid_thw is not None:
        print(f"[DEBUG] video_grid_thw BEFORE transfer - device: {video_grid_thw.device}, dtype: {video_grid_thw.dtype}, shape: {video_grid_thw.shape}")
    # ==================================

    # 在函数开始处统一处理设备转移
    device = input_ids.device
    if image_grid_thw is not None:
        image_grid_thw = image_grid_thw.to(device=device, dtype=torch.long).contiguous()
        print(f"[DEBUG] image_grid_thw AFTER transfer - device: {image_grid_thw.device}, dtype: {image_grid_thw.dtype}")
    if video_grid_thw is not None:
        video_grid_thw = video_grid_thw.to(device=device, dtype=torch.long).contiguous()
        print(f"[DEBUG] video_grid_thw AFTER transfer - device: {video_grid_thw.device}, dtype: {video_grid_thw.dtype}")
    ...
    if pixel_values is not None:
        ...
        print(f"[DEBUG] About to call model.visual with image_grid_thw device: {image_grid_thw.device if image_grid_thw is not None else 'None'}")
        # 调用前再次确认
        if image_grid_thw is not None and image_grid_thw.device != pixel_values.device:
            print(f"[WARNING] Device mismatch detected! Forcing transfer...")
        ...
        print(f"[DEBUG] model.visual call succeeded for images")
    ...
    if pixel_values_videos is not None:
        ...
        print(f"[DEBUG] pixel_values_videos device: {pixel_values_videos.device}")
        print(f"[DEBUG] About to call model.visual with video_grid_thw device: {video_grid_thw.device if video_grid_thw is not None else 'None'}")
        # 调用前再次确认
        if video_grid_thw is not None and video_grid_thw.device != pixel_values_videos.device:
            print(f"[WARNING] Device mismatch detected for video! Forcing transfer...")
        ...
        print(f"[DEBUG] model.visual call succeeded for videos")
    ...
    print(f"[DEBUG] _get_input_embeds completed successfully")
