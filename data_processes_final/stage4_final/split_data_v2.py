...
"""
Split final v2 data into train (90%) and val (10%) subsets.
Reads {split}_final_v2.json and outputs {split}_final_v2_09 and {split}_final_v2_01.
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Split final v2 data into train/val subsets")
    parser.add_argument("--split", required=True, help="Split name (e.g., train, val, test)")
    parser.add_argument("--input_dir", default=None, help="Input directory containing final v2 data")
    parser.add_argument("--output_dir", default=None, help="Output directory for split v2 data")
    ...
    # Set default paths using relative paths (v2 uses data_duo_final_v2)
    if args.input_dir is None:
        args.input_dir = verl_duo_dir / "data_duo_final_v2" / "ChartQA_2000"
    else:
        args.input_dir = Path(args.input_dir)

    if args.output_dir is None:
        args.output_dir = verl_duo_dir / "data_duo_final_v2" / "ChartQA_2000"
    else:
    ...
    # Load final v2 data
    input_file = args.input_dir / f"{args.split}_final_v2.json"
    ...
    # Output suffixes (v2)
    train_suffix = f"{args.split}_final_v2_09"
    val_suffix = f"{args.split}_final_v2_01"
