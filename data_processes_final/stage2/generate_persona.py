...
def main() -> None:
    ...
    if args.input_json is None:
        args.input_json = verl_duo_dir / "data_duo_final" / "ChartQA_2000" / f"{args.split}.json"
    else:
        args.input_json = Path(args.input_json)

    if args.input_parquet is None:
        args.input_parquet = verl_duo_dir / "data_duo_final" / "ChartQA_2000" / f"{args.split}.parquet"
    else:
        args.input_parquet = Path(args.input_parquet)

    if args.output_json is None:
        args.output_json = verl_duo_dir / "data_duo_final" / "ChartQA_2000" / f"{args.split}_with_persona.json"
    else:
        args.output_json = Path(args.output_json)

    if args.output_parquet is None:
        args.output_parquet = verl_duo_dir / "data_duo_final" / "ChartQA_2000" / f"{args.split}_with_persona.parquet"
    else:
