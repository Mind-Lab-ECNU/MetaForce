#!/usr/bin/env python3
"""Generate personas and pref_vec values with ChatECNU (v2, tool-aware)."""
...
def _load_tools(tool_json_path: Path) -> List[Dict[str, Any]]:
    """Load tool definitions from JSON."""
    with tool_json_path.open("r", encoding="utf-8") as f:
        return json.load(f)


def _is_executable_tool(tool: Dict[str, Any]) -> bool:
    """Executable tools do not have the model parameter."""
    props = tool.get("function", {}).get("parameters", {}).get("properties", {})
    return "model" not in props


def _build_pref_vec(tools: List[Dict[str, Any]], style_key: str) -> Dict[str, float]:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate personas via ChatECNU (v2).")
    parser.add_argument(
        "--split",
        default="train",
        help="Split name (e.g., train, test, validation).",
    )
    parser.add_argument(
        "--input_json",
        default=None,
    ...
    parser.add_argument(
        "--tool_json",
        default=None,
        help="Path to tool definitions for building pref_vec.",
    )
    parser.add_argument(
        "--api_key",
        default="a",
    ...
    parser.add_argument(
        "--base_proxy_url",
        default="https://notebook-inspire.sii.edu.cn/ws-9dcc0e1f-80a4-4af2-bc2f-0e352e7b17e6/project-b795c114-135a-40db-b3d0-19b60f25237b/user-c6264b38-46a1-4bb6-a3a7-1db39453f6f8/vscode/60a27891-894e-4137-9683-3ec973d46202/9c2d8a07-0c9d-429c-a6b3-e0cf78a5d17c/proxy/8086/",
...
def process_sample(
    ...
    item["extra_info"]["style"] = style_key
    item["extra_info"]["pref_vec"] = _build_pref_vec(tools, style_key)
...
def main() -> None:
    ...
    if args.input_json is None:
        args.input_json = verl_duo_dir / "data_duo_final_v2" / "ChartQA_2000" / f"{args.split}.json"
    else:
        args.input_json = Path(args.input_json)

    if args.input_parquet is None:
        args.input_parquet = verl_duo_dir / "data_duo_final_v2" / "ChartQA_2000" / f"{args.split}.parquet"
    else:
        args.input_parquet = Path(args.input_parquet)

    if args.output_json is None:
        args.output_json = verl_duo_dir / "data_duo_final_v2" / "ChartQA_2000" / f"{args.split}_with_persona_v2.json"
    else:
        args.output_json = Path(args.output_json)

    if args.output_parquet is None:
        args.output_parquet = verl_duo_dir / "data_duo_final_v2" / "ChartQA_2000" / f"{args.split}_with_persona_v2.parquet"
    else:
        args.output_parquet = Path(args.output_parquet)

    if args.tool_json is None:
        args.tool_json = verl_duo_dir / "real_tool.json"
    else:
        args.tool_json = Path(args.tool_json)
    ...
    tools = _load_tools(Path(args.tool_json))
    ...
    for idx, item in enumerate(data_json[:total]):
        ...
        persona_zh, persona_en = process_sample(
            item,
            style_key=style_key,
            base_proxy_url=args.base_proxy_url,
            timeout=args.timeout,
            use_mock=args.mock_persona,
            tools=tools,
        )
